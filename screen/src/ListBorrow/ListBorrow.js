...
class ListBorrow extends Component {
    ...
    componentDidMount() {
        ...
        fetch("http://125.253.123.20/managedevice/group.php", {
            ...
            .then((response) => {
                this.setState({ mang: response.DANHSACHSANPHAMDAMUON })
                response.DANHSACHSANPHAMDAMUON.map((value)=> {
                    var d1 = Date.parse(value.return_date);
                    var d2 = Date.now();
                    if (d1>d2)
                    PushNotification.localNotificationSchedule({
                        id:value.deviceId,
                        message: "Notice: Device borrowing time is expired",
                        date: new Date(Date.now() + (d1-d2)) // in 60 secs
                    })
                    // if(d1-d2==1) 
                    // PushNotification.localNotificationSchedule({
                    //     id:value.deviceId,
                    //     message: "Còn 1 ngày đến hạn nộp thiết bị",
                    //     date: new Date(Date.now() + (d1-d2==1)) // in 60 secs
                    // })
                })
            })
    }
